...
public:
  ...
  // Constructor. Links the PID to Input, Output, Setpoint, initial tuning
  // parameters and control modes.
  QuickPID(double *Input, double *Output, double *Setpoint, double Kp,
           double Ki, double Kd, pMode pMode, dMode dMode, iAwMode iAwMode,
           Action Action);

  // Overload constructor links the PID to Input, Output, Setpoint, tuning
  // parameters and control Action. Uses defaults for remaining parameters.
  QuickPID(double *Input, double *Output, double *Setpoint, double Kp,
  ...
  // Sets PID mode to manual (0), automatic (1), timer (2) or toggle
  // manual/automatic (3).
  void SetMode(Control Mode);
  void SetMode(uint8_t Mode);

  // Performs the PID calculation. It should be called every time loop() cycles
  // ON/OFF and calculation frequency can be set using SetMode and SetSampleTime
  // respectively.
  bool Compute();
  ...
  void SetTunings(double Kp, double Ki, double Kd, pMode pMode, dMode dMode,
                  iAwMode iAwMode);

  // Sets the controller direction or action. Direct means the output will
  // increase when the error is positive. Reverse means the output will decrease
  // when the error is positive.
  void SetControllerDirection(Action Action);
  void SetControllerDirection(uint8_t Direction);

  // Sets the sample time in microseconds with which each PID calculation is
  // performed. Default is 100000 µs.
  ...
  // either on error or measurement (default).
  void SetDerivativeMode(dMode dMode);
  void SetDerivativeMode(uint8_t Dmode);

  // Sets the integral anti-windup mode to one of iAwClamp, which clamps the
  // output after adding integral and proportional (on measurement) terms, or
  // iAwCondition (default), which provides some integral correction, prevents
  // deep saturation and reduces overshoot. Option iAwOff disables anti-windup
  // altogether.
  ...
  uint8_t GetMode();      // manual (0), automatic (1), timer (2) or toggle
                          // manual/automatic (3)
...
private:
  ...
  double *myInput;  // Pointers to the Input, Output, and Setpoint variables.
                    // This creates a
  double *myOutput; // hard link between the variables and the PID, freeing the
                    // user from having
  double *mySetpoint; // to constantly tell us what these values are. With
                      // pointers we'll just know.
