...
/* Constructor
 ******************************************************************** The
 parameters specified here are those for for which we can't set up reliable
...
/* Constructor
 ********************************************************************* To allow
 using pOnError, dOnMeas and iAwCondition without explicitly saying so.
 **********************************************************************************/
QuickPID::QuickPID(double *Input, double *Output, double *Setpoint, double Kp,
                   double Ki, double Kd, Action Action)
    : QuickPID::QuickPID(Input, Output, Setpoint, Kp, Ki, Kd,
                         pmode = pMode::pOnError, dmode = dMode::dOnMeas,
                         iawmode = iAwMode::iAwCondition, action = Action) {}
...
QuickPID::QuickPID(double *Input, double *Output, double *Setpoint)
    : QuickPID::QuickPID(
          Input, Output, Setpoint, dispKp = 0, dispKi = 0, dispKd = 0,
          pmode = pMode::pOnError, dmode = dMode::dOnMeas,
          iawmode = iAwMode::iAwCondition, action = Action::direct) {}

/* Compute()
 *********************************************************************** This
 function should be called every time "void loop()" executes. The function will
 decide whether a new PID Output needs to be computed. Returns true when the
...
bool QuickPID::Compute() {
  ...
  if (mode == Control::timer || timeChange >= sampleTimeUs) {
    ...
    if (iawmode == iAwMode::iAwOff)
      ...
      outputSum = constrain(outputSum - pmTerm, outMin,
                            outMax); // include pmTerm and clamp
    *myOutput = constrain(outputSum + peTerm + dTerm, outMin,
                          outMax); // include dTerm, clamp and drive output
...
void QuickPID::SetMode(Control Mode) {
  if (mode == Control::manual &&
      Mode != Control::manual) { // just went from manual to automatic, timer or
                                 // toggle
    QuickPID::Initialize();
  }
...
void QuickPID::SetMode(uint8_t Mode) {
  if (mode == Control::manual &&
      Mode != 0) { // just went from manual to automatic or timer
    QuickPID::Initialize();
  }
  if (Mode == 3) { // toggle
    mode = (mode == Control::manual) ? Control::automatic : Control::manual;
...
******************************************************************************/
void QuickPID::SetControllerDirection(Action Action) { action = Action; }
...
******************************************************************************/
void QuickPID::SetProportionalMode(pMode pMode) { pmode = pMode; }
void QuickPID::SetProportionalMode(uint8_t Pmode) { pmode = (pMode)Pmode; }

/* SetDerivativeMode(.)*******************************************************
  Sets the computation method for the derivative term, to compute based
  either on error or on measurement (default).
******************************************************************************/
void QuickPID::SetDerivativeMode(dMode dMode) { dmode = dMode; }
void QuickPID::SetDerivativeMode(uint8_t Dmode) { dmode = (dMode)Dmode; }
...
******************************************************************************/
void QuickPID::SetAntiWindupMode(iAwMode iAwMode) { iawmode = iAwMode; }
...
void QuickPID::SetOutputSum(double sum) { outputSum = sum; }

/* Status Functions************************************************************
  These functions query the internal state of the PID.
******************************************************************************/
double QuickPID::GetKp() { return dispKp; }
double QuickPID::GetKi() { return dispKi; }
double QuickPID::GetKd() { return dispKd; }
double QuickPID::GetPterm() { return pTerm; }
double QuickPID::GetIterm() { return iTerm; }
double QuickPID::GetDterm() { return dTerm; }
double QuickPID::GetOutputSum() { return outputSum; }
uint8_t QuickPID::GetMode() { return static_cast<uint8_t>(mode); }
uint8_t QuickPID::GetDirection() { return static_cast<uint8_t>(action); }
uint8_t QuickPID::GetPmode() { return static_cast<uint8_t>(pmode); }
uint8_t QuickPID::GetDmode() { return static_cast<uint8_t>(dmode); }
uint8_t QuickPID::GetAwMode() { return static_cast<uint8_t>(iawmode); }
